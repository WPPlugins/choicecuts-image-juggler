...
Remove all images from passed string/content
...
Make filters available by template selection
Pass parameter options into Filter functions
Upgrade thumbnail.inc.php to most recent library version, v.3
...
// Enable filter on the_content  - resize images if width outside set values. N.B. edit variable vaules within the actual funciton to specify exact requirements.
define('FILTER_FLEXI_WIDTH', FALSE);

// Enable filter on the_content  - all images cropped. N.B. edit variable vaules within the actual funciton to specify exact requirements.
define('FILTER_ALL_CROP', FALSE);

// Enable filter on the_content  - all images width resize. N.B. edit variable vaules within the actual funciton to specify exact requirements.
define('FILTER_ALL_WIDTH', FALSE);

// Alter this path if you wish to use a different default image
define('DEFAULT_IMAGE', get_bloginfo('template_directory').'/images/default_image.jpg');
...
define('PLUGIN_PATH', WP_PLUGIN_URL .'/cc_image_juggler/includes/');
define('LIGHTBOX_PATH', WP_PLUGIN_URL .'/cc_image_juggler/includes/lightbox/'.$lbDirectory.'/');
...
// -------------------------------------------------------------------------- INTERNAL PLUGIN UTILITIES
	...
	*/
	...
	{
		$str = '<script src="'.PLUGIN_PATH.'jquery.js" type="text/javascript"></script>';
		switch ( LIGHTBOX_CHOICE ) {
			case 1:
				$str .= "
		...
		if ( $link != FALSE ) {
			if ($link === TRUE) {
				if (LIGHTBOX) {
					...
					switch ( LIGHTBOX_CHOICE ) {
						case 1:
							return $html_partial = "<a href='".ccImj_maxDims( $img_path, LIGHTBOX_MAX_WIDTH, LIGHTBOX_MAX_HEIGHT )."' $lightbox>".$html_partial."</a>";
							
						case 2:
							return $html_partial = "<span class='single thumbs_all'><a href='$img_path' $lightbox>".$html_partial."</a></span>";
					}
				}
			}
			else {
...
// -------------------------------------------------------------------------- IMAGE MANIPULATION PREPARATION
	...
	*/
	function ccImj_resizeWidth( $img_path, $width=NULL, $extraTags=NULL )
	{
		...
		$html_partial .= "<img src='".PLUGIN_PATH."$script?img=$img_name$hor_dim$vert_dim' $tag_dims $extraTags>";
	...
	* @return html_partial: string
	*/
	function ccImj_cropSquareCtr( $img_path, $size=NULL, $extraTags=NULL )
	{
		$img_dim = NULL;
		$html_partial = NULL;
		$img_name = ccImj_abs2rel( $img_path );		
...
// -------------------------------------------------------------------------- GET FIRST FUNCTIONS
	...
	* @return html_partial: string. send to output or returned as variable depending on @param: return
	*/	
	function ccImj_firstThumb( $content, $size=NULL, $link=FALSE, $return=FALSE, $extraTags=NULL )
	{
		$img_path = ccImj_getFirstImg( $content );
...
// -------------------------------------------------------------------------- RESIZE & CROP PASSED IMAGE FUNCTIONS
	...
	* @param img_path: string - filename path
	* @param width: integer - width dimension
	* @param min_height: integer - min. height dimension
	* @param link: boolean/string - lighbox integrate or hyperlink URL
	* @param return: boolean - output conditional
	* @param extraTags: string - open to include any additional HTML attribute tags
	* @return html_partial: string. send to output or returned as variable depending on @param: return
	*/
	function ccImj_linkedLandscape( $img_path, $width, $min_height, $link=FALSE, $return=FALSE, $extraTags=NULL )
	{
	...
	* @return html_partial: string. send to output or returned as variable depending on @param: return
	*/
	function ccImj_linkedPortrait( $img_path, $height, $width, $link=FALSE, $return=FALSE, $extraTags=NULL )
	{
		$html_partial = ccImj_cropPortrait( $img_path, $height, $width, $extraTags );
		$html_partial = ccImj_addLink( $html_partial, $link, $img_path );
	...
	* @return html_partial: string. send to output or returned as variable depending on @param: return
	*/
	function ccImj_linkedThumb( $img_path, $size=NULL, $link=FALSE, $return=FALSE, $extraTags=NULL )
	{
		$html_partial = ccImj_cropSquareCtr( $img_path, $size, $extraTags );
		$html_partial = ccImj_addLink( $html_partial, $link, $img_path );
...
// -------------------------------------------------------------------------- FILTER ALL FUNCTIONS
	...
	*/
			...
			foreach($images AS $image) {
				...
				if ( ccImj_isLocal( $image['url'] )) {
					...
					if ($img_width > $max_width) {
						...
						$html_partial = ccImj_addLink( $html_partial, $link, $image['url'] );
	...
	if (FILTER_FLEXI_WIDTH && ! FILTER_ALL_WIDTH && ! FILTER_ALL_CROP) {
		add_filter('the_content', 'ccImj_flexiWidth', 1, 1);
	}
	...
	*/
	...
	if (FILTER_ALL_WIDTH && ! FILTER_FLEXI_WIDTH && ! FILTER_ALL_CROP) {
		add_filter('the_content', 'ccImj_allWidth', 1, 1);
	}
	...
	*/
	...
	if (FILTER_ALL_CROP && ! FILTER_FLEXI_WIDTH && ! FILTER_ALL_WIDTH) {
		add_filter('the_content', 'ccImj_allCrop', 1, 1);
	}
